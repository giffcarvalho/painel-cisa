...
export const StatusBadge = ({ ativo }) => <span className={`${styles.badge} ${ativo ? styles.success : styles.muted}`}>{ativo ? 'Ativo' : 'Desativado'}</span>
export const AccountBadge = ({ ativa }) => <span className={`${styles.badge} ${ativa ? styles.info : styles.warning}`}>{ativa ? 'Ativada' : 'Aguardando ativação'}</span>
const data = (valor) => valor ? new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(valor)) : 'Nunca acessou'

export default function UsersTable({ usuarios, onSelect }) {
  ...
  return <div className={styles.tableWrap}><table className={styles.table}>
    ...
    <tbody>{usuarios.map((usuario) => <tr key={usuario.id_usuario} onClick={() => onSelect(usuario.id_usuario)}>
      ...
      <td>{usuario.perfil === 'admin' ? 'Administrador' : 'Técnico'}</td><td><StatusBadge ativo={usuario.ativo} /></td><td><AccountBadge ativa={usuario.conta_ativada} /></td>
