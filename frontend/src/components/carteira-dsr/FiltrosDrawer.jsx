    ...
    <div className="relative flex flex-col gap-1 mb-2">
      ...
      {aberto && (
        <div className="absolute left-0 right-0 top-full z-[60] mt-1 max-h-56 overflow-y-auto rounded-md border border-gray-200 bg-white py-1 text-sm shadow-lg">
          ...
          <button
            type="button"
            className="w-full px-3 py-2 text-left font-medium text-red-600 hover:bg-red-50 text-xs border-b border-gray-100"
            onMouseDown={(e) => e.preventDefault()}
            onClick={() => handleSelecionar('')}
          >
            Limpar seleção deste campo
          </button>
          ...
          {opcoesFiltradas.map((item) => {
            ...
            return (
              <button
                type="button"
                key={item}
                className={`w-full px-3 py-2 text-left text-sm flex items-center gap-2 hover:bg-blue-50 ${selecionado ? 'bg-blue-50 font-medium text-blue-700' : 'text-gray-700'}`}
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => handleSelecionar(item)}
              >
                <div className={`w-4 h-4 border rounded flex items-center justify-center shrink-0 transition-colors ${selecionado ? 'bg-blue-600 border-blue-600' : 'border-gray-400'}`}>
                  {selecionado && (
