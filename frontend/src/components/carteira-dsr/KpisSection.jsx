import { Loader, AlertCircle, Loader2 } from 'lucide-react'
...
export default function KpisSection() {
    ...
    if(isLoading) {
        return (
            <div className="flex w-full h-24 items-center justify-center bg-white rounded-lg border border-gray-200 shadow-sm">
                ...
                <span className="ml-3 text-sm font-medium text-gray-500">Processando indicadores da carteira...</span>
            </div>
        )
    }
    if(isError) {
        return (
            <div className="flex w-full h-24 items-center justify-center bg-red-50 rounded-lg border border-red-200">
                ...
                <span className="ml-3 text-sm font-medium text-red-700">
                    Falha ao carregar os indicadores. Ajuste os filtros ou tente novamente mais tarde.
                </span>
            </div>
        )
    }
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3">
            ...
            <KpiCard titulo="Desembolsado" valor={formatCurrency(kpis.valor_desemboslsado, true)} />
            <KpiCard titulo="Desbloqueado" valor={formatCurrency(kpis.valor_desbloqueado, true)} />
            <KpiCard titulo="Municípios" valor={kpis.qtde_municipios_beneficiados} />
        </div>
    )
}
