...
MV_MUNICIPIOS = "instrumento.vw_instrumento_municipio"
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "ao consultar a base de dados."
            ),
        )


def _build_where(
    filtros: FiltrosPontosControle,
) -> tuple[str, dict]:
    ...
    if filtros.municipios_beneficiados:
        ...
        for i, value in enumerate(
            filtros.municipios_beneficiados
        ):
    ...
    list_filters = [
        ...
        (
            "carteira_ativa",
            filtros.carteira_ativa,
            "carteira_ativa",
            None,
        ),
        (
            "projeto_aprovado",
            filtros.projeto_aprovado,
            "projeto_aprovado",
            None,
        ),
        (
        ...
        (
            ...
            filtros.prazo_clausulas_suspensivas,
            "prazo_clausulas_suspensivas",
            None,
        ),
        (
            "prazo_emissao_lae",
            filtros.prazo_emissao_lae,
            "prazo_emissao_lae",
            None,
        ),
        (
            "prazo_inicio_licitacao",
            filtros.prazo_inicio_licitacao,
            "prazo_inicio_licitacao",
            None,
        ),
        (
            "prazo_conclusao_licitacao",
            filtros.prazo_conclusao_licitacao,
            "prazo_conclusao_licitacao",
            None,
        ),
        (
        ...
        (
            "prazo_contratacao",
            filtros.prazo_contratacao,
            "prazo_contratacao",
            None,
        ),
        (
            "prazo_solicitacao_aio",
            filtros.prazo_solicitacao_aio,
            "prazo_solicitacao_aio",
            None,
        ),
        (
            "prazo_analise_tecnica_aio",
            filtros.prazo_analise_tecnica_aio,
            "prazo_analise_tecnica_aio",
            None,
        ),
        (
            "prazo_analise_executiva_aio",
            filtros.prazo_analise_executiva_aio,
            "prazo_analise_executiva_aio",
            None,
        ),
        (
            "prazo_registro_aio",
            filtros.prazo_registro_aio,
            "prazo_registro_aio",
            None,
        ),
        (
            "prazo_emissao_os",
            filtros.prazo_emissao_os,
            "prazo_emissao_os",
            None,
        ),
        (
            "prazo_inicio_execucao_fisica",
            filtros.prazo_inicio_execucao_fisica,
            "prazo_inicio_execucao_fisica",
            None,
        ),
        (
            "prazo_progresso_fisico",
            filtros.prazo_progresso_fisico,
            "prazo_progresso_fisico",
            None,
        ),
        (
            "prazo_indicio_paralisacao",
            filtros.prazo_indicio_paralisacao,
            "prazo_indicio_paralisacao",
            None,
        ),
        (
            "status_paralisacao_obra",
            filtros.status_paralisacao_obra,
            "status_paralisacao_obra",
            None,
        ),
        (
            "vistoria_in_loco_parciais",
            filtros.vistoria_in_loco_parciais,
            "vistoria_in_loco_parciais",
            None,
        ),
        (
            "prazo_vistoria_final",
            filtros.prazo_vistoria_final,
            "prazo_vistoria_final",
            None,
        ),
        (
            "obras_proximas_conclusao",
            filtros.obras_proximas_conclusao,
            "obras_proximas_conclusao",
            None,
        ),
        (
            "registro_conclusao",
            filtros.registro_conclusao,
            "registro_conclusao",
            None,
        ),
        (
        ...
        (
            ...
            filtros.status_de_execucao_da_obra,
            "status_de_execucao_da_obra",
            None,
        ),
    ]
...
@router.get(
    "/filtros",
    response_model=PontosControleFiltrosResponse,
)
async def get_filtros(
    ...
    sql = f"""
        ...
        municipios AS (
            SELECT json_agg(
                json_build_object(
                    'cod_municipio', m.cod_municipio::text,
                    'municipio', m.nome
                )
                ORDER BY m.nome
            ) AS lista
        ...
        SELECT
            ...
            COALESCE(
                array_agg(
                    DISTINCT nr_instrumento::text
                    ORDER BY nr_instrumento::text
                )
                FILTER (
                    WHERE nr_instrumento::text IS NOT NULL
                ),
                ARRAY[]::text[]
            ) AS nr_instrumento,
            ...
            COALESCE(
                array_agg(
                    DISTINCT carteira_ativa
                    ORDER BY carteira_ativa
                )
                FILTER (
                    WHERE carteira_ativa IS NOT NULL
                ),
                ARRAY[]::text[]
            ) AS carteira_ativa,
            ...
            COALESCE(
                array_agg(
                    DISTINCT projeto_aprovado
                    ORDER BY projeto_aprovado
                )
                FILTER (
                    WHERE projeto_aprovado IS NOT NULL
                ),
                ARRAY[]::text[]
            ) AS projeto_aprovado,
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_clausulas_suspensivas
                    ORDER BY prazo_clausulas_suspensivas
                )
                FILTER (
                    WHERE prazo_clausulas_suspensivas IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_emissao_lae
                    ORDER BY prazo_emissao_lae
                )
                FILTER (
                    WHERE prazo_emissao_lae IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_inicio_licitacao
                    ORDER BY prazo_inicio_licitacao
                )
                FILTER (
                    WHERE prazo_inicio_licitacao IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_conclusao_licitacao
                    ORDER BY prazo_conclusao_licitacao
                )
                FILTER (
                    WHERE prazo_conclusao_licitacao IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_contratacao
                    ORDER BY prazo_contratacao
                )
                FILTER (
                    WHERE prazo_contratacao IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_solicitacao_aio
                    ORDER BY prazo_solicitacao_aio
                )
                FILTER (
                    WHERE prazo_solicitacao_aio IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_analise_tecnica_aio
                    ORDER BY prazo_analise_tecnica_aio
                )
                FILTER (
                    WHERE prazo_analise_tecnica_aio IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_analise_executiva_aio
                    ORDER BY prazo_analise_executiva_aio
                )
                FILTER (
                    WHERE prazo_analise_executiva_aio IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_registro_aio
                    ORDER BY prazo_registro_aio
                )
                FILTER (
                    WHERE prazo_registro_aio IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_emissao_os
                    ORDER BY prazo_emissao_os
                )
                FILTER (
                    WHERE prazo_emissao_os IS NOT NULL
                ),
                ARRAY[]::text[]
            ) AS prazo_emissao_os,
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_inicio_execucao_fisica
                    ORDER BY prazo_inicio_execucao_fisica
                )
                FILTER (
                    WHERE prazo_inicio_execucao_fisica IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_progresso_fisico
                    ORDER BY prazo_progresso_fisico
                )
                FILTER (
                    WHERE prazo_progresso_fisico IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_indicio_paralisacao
                    ORDER BY prazo_indicio_paralisacao
                )
                FILTER (
                    WHERE prazo_indicio_paralisacao IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT status_paralisacao_obra
                    ORDER BY status_paralisacao_obra
                )
                FILTER (
                    WHERE status_paralisacao_obra IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT vistoria_in_loco_parciais
                    ORDER BY vistoria_in_loco_parciais
                )
                FILTER (
                    WHERE vistoria_in_loco_parciais IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT prazo_vistoria_final
                    ORDER BY prazo_vistoria_final
                )
                FILTER (
                    WHERE prazo_vistoria_final IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT obras_proximas_conclusao
                    ORDER BY obras_proximas_conclusao
                )
                FILTER (
                    WHERE obras_proximas_conclusao IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT registro_conclusao
                    ORDER BY registro_conclusao
                )
                FILTER (
                    WHERE registro_conclusao IS NOT NULL
                ),
                ARRAY[]::text[]
            ...
            COALESCE(
                array_agg(
                    DISTINCT status_de_execucao_da_obra
                    ORDER BY status_de_execucao_da_obra
                )
                FILTER (
                    WHERE status_de_execucao_da_obra IS NOT NULL
                ),
                ARRAY[]::text[]
    ...
    return PontosControleFiltrosResponse(
...
@router.get(
    "/filtros/busca",
    response_model=PontosControleBuscaFiltroResponse,
    summary="Busca de filtros dos Pontos de Controle",
)
async def buscar_opcoes_filtro(
    campo: Annotated[
        str,
        Query(
            description="Campo pesquisável."
        ),
    ],
    q: Annotated[
        str,
...
@router.get(
    "/instrumentos",
    response_model=PontosControleListaResponse,
    summary="Lista instrumentos dos Pontos de Controle",
)
async def get_instrumentos(
    ...
    count_result = await _execute_query(
        db,
        count_sql,
        params,
    )

    data_result = await _execute_query(
        db,
        data_sql,
        data_params,
    )
    ...
    return PontosControleListaResponse(
        ...
        data=[
            dict(r)
            for r in data_result.mappings().all()
        ],
    )
